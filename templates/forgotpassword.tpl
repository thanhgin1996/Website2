...
<head>
	<meta charset="utf-8">
		<title><?php echo $description ?> - Đăng Ký</title>
...
<div class="main-content">
			...
			<div class="card-body p-0 m-0">
				<div class="row m-0 p-0">
					<div class="col-lg-5 p-0 d-none d-lg-block">
						<img src="/assets/themes/lost-theme/images/auth.jpg" class="img-fluid">
					</div>
    ...
    <form name="regsiter" id="regsiter" action="" method="post">         	<input name="regsiter" value="regsiter" type="hidden">
	<div class="mb-3">
        ...
        <div class="mb-3">
                    <label class="form-label"><label for="password2">Mật Khẩu Cấp 2</label></label>
        
                    <input name="password2" placeholder="Nhập Mật Khẩu Cấp 2" class="form-control" type="password" value="">            </div>
		  <div class="mb-3">
                    <label class="form-label"><label for="password">Mật Khẩu Mới</label></label>
        
                    <input name="passwordnew" placeholder="Nhập Mật Khẩu Mới" class="form-control" type="password" value="">            </div>		
<div class="mb-3">
                    <label class="form-label"><label for="password">Nhập Lại Mật Khẩu Mới</label></label>
        
                    <input name="repasswordnew" placeholder="Nhập Lại Mật Khẩu Mới" class="form-control" type="password" value="">            </div>						
         <div class="mb-3">
        </br><div class="image-container">
            <img src="/img/imgcaptcha.png" alt="Captcha Image">
            <div class="center-text"><?php echo $newcaptcha ?></div> <!-- Text ở giữa ảnh -->
        </div>
        <label for="captcha" class="form-label">
            Nhập Mã Captcha Phía Trên.<span class="required-mark">*</span>
        </label>
        <input name="captcha" placeholder="" class="form-control" type="text" required="" value="">
        <input name="captcha2" placeholder="" class="form-control" type="hidden" required="" value="<?php echo $newcaptcha ?>">
    </div>
		<div class="mb-3">
        
            <button class="btn btn-primary" type="submit" name="Submit" value="">Xác Nhận</button> 
</form>
										 </div>
									</div>
								</div>
...
<script>
    (function() {
        const imageText = document.querySelector('.center-text');

        // Giá trị Captcha từ server (mã PHP sẽ chèn tại đây)
        const captchaText = "<?php echo $newcaptcha; ?>";

        // Hàm kiểm tra DevTools
        function detectDevTools() {
            const threshold = 160; // Ngưỡng kích thước để nhận biết DevTools mở
            const isDevToolsOpen = window.outerWidth - window.innerWidth > threshold || 
                                   window.outerHeight - window.innerHeight > threshold;

            if (isDevToolsOpen) {
                // Nếu mở DevTools, thay đổi style
                if (imageText) {
                    imageText.style.color = 'transparent'; // Ẩn text
                    imageText.style.textShadow = 'none'; // Loại bỏ shadow nếu có
                    imageText.innerText = ''; // Xóa nội dung text
                }
            } else {
                // Nếu đóng DevTools, khôi phục giá trị ban đầu
                if (imageText) {
                    imageText.style.color = ''; // Khôi phục màu chữ
                    imageText.style.textShadow = ''; // Khôi phục shadow nếu cần
                    imageText.innerText = captchaText; // Đặt lại giá trị Captcha
                }
            }
        }

        // Lắng nghe sự kiện resize để phát hiện
        window.addEventListener('resize', detectDevTools);

        // Kiểm tra trạng thái DevTools ngay khi tải trang
        detectDevTools();
    })();
</script>
